...
# ///
"""
Pre-tool use hook for Claude Code to guide pnpm usage in Node.js/TypeScript projects.
...
class PnpmCommandHandler:
    ...
    def _transform_single_command(self, command: str) -> str:
        ...
        # npm install
        if re.match(r"^npm\s+install\s*$", command) or re.match(r"^npm\s+i\s*$", command):
            return "pnpm install"

        # npm install <package>
        elif re.match(r"^npm\s+install\s+", command) or re.match(r"^npm\s+i\s+", command):
        ...
        # npm uninstall
        elif re.match(r"^npm\s+uninstall\s+", command) or re.match(r"^npm\s+remove\s+", command):
        ...
        elif re.match(r"^yarn\s+install\s*$", command) or re.match(r"^yarn\s*$", command):
            return "pnpm install"
        ...
        return command.replace("npm", "pnpm").replace("npx", "pnpm exec").replace("yarn", "pnpm")
